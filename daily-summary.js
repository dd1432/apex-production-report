...
function machinesOf(report) {
    return Array.isArray(report.machines) ? report.machines : Object.values(report.machines || {});
}
...
function renderDailyTable() {
    ...
    let reportCount = 0;
    reports.filter(report => report.productionDate === selectedDate && report.unit === selectedUnit).forEach(report => {
    ...
    const rows = Object.entries(processMachines).map(([process, machines]) => {
        ...
        return `<tr><td>${process} Total</td>${shifts.map(shift => `<td>${format(shiftTotals[shift].length)} m</td><td>${format(shiftTotals[shift].weight)} kg</td>`).join("")}<td class="total-column">${format(processTotal.length)} m</td><td class="total-column">${format(processTotal.weight)} kg</td></tr>`;
    }).join("");
    ...
    container.innerHTML = `<div class="daily-table-wrapper"><table class="daily-table"><thead><tr><th rowspan="2">Production</th>${shiftHeadings}<th class="total-column" colspan="2">All Shift Total</th></tr><tr>${unitHeadings}<th class="unit-heading total-column">Mtr</th><th class="unit-heading total-column">Kg</th></tr></thead><tbody>${rows}</tbody></table></div>`;
}
